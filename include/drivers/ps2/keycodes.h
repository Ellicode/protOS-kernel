#ifndef PS2_KEYCODES_H
#define PS2_KEYCODES_H
...
typedef struct kbd_keyset {
    const char *name;
    const unsigned char *unshifted;
    const unsigned char *shifted;
    const unsigned char *altgr;
} kbd_keyset_t;

extern const kbd_keyset_t *kbd_active_keyset;
extern const kbd_keyset_t kbd_keyset_csa;

void kbd_set_keyset(const kbd_keyset_t *keyset);
const kbd_keyset_t *kbd_get_keyset(void);

static inline unsigned char kbd_to_char(kbd_keycode_t keycode, uint8_t modifiers) {
    if (modifiers & KBD_MOD_ALTGR) {
        return kbd_active_keyset->altgr[keycode];
    }
    if (modifiers & KBD_MOD_SHIFT) {
        return kbd_active_keyset->shifted[keycode];
    }
    return kbd_active_keyset->unshifted[keycode];
}

extern const kbd_keycode_t ps2_scancode_to_keycode[256];
extern const kbd_keycode_t ps2_ext_scancode_to_keycode[256];

extern const kbd_keycode_t ascii_to_kbd_keycode[128];

#endif // PS2_KEYCODES_H
