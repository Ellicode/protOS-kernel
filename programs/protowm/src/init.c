...
int wm_init() {
    ...
    if (g_fb == NULL) {
        fprintf(STDOUT, "[ERROR] Cannot allocate framebuffer\n");
        return 1;
    }

    int fb_res = fetch_framebuffer(g_fb);
    if (fb_res != PROTO_OK) {
        fprintf(STDOUT, "[ERROR] Cannot request framebuffer\n");
        free(g_fb);
        return 1;
    }

    g_cursor_bmp = bmp_load("/system/assets/img/cursor.bmp");
    ...
    for (int i = 0; i < (sizeof(subs) / sizeof(subs[0])); i++) {
        if (subscribe(subs[i]) != PROTO_OK) {
            fprintf(STDOUT, "[ERROR] Subscribe failed -> \"%s\"", subs[i]);
            free(g_fb);
            return 1;
        }   
    }
